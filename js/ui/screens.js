...
function updateHeader() {
  ...
  // Layout in the right slot of the header: sign-in on the LEFT,
  // sign-out in the MIDDLE, fullscreen (expand/collapse icon)
  // next, settings (gear) on the RIGHT. (Cursor settings was
  // repurposed into the gear icon — no second settings button
  // needed.)
  right.appendChild(signInBtn);
  right.appendChild(signOutBtn);
  // Fullscreen button — toggles the browser fullscreen API on
  // the document element. Sits just to the LEFT of the settings
  // gear icon, so it's reachable from every screen without the
  // user needing to enter the game first. Uses the same
  // .fullscreen-icon-btn class so the global fullscreenchange
  // listener (in helpers.js) keeps the expand/compress icon
  // in sync.
  right.appendChild(fullscreenButton());
